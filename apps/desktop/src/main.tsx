...
import type {Appearance,SourceFile,SavedFile} from './contracts';
import './style.css';
function App(){
  ...
  return <main>
    ...
    <section className="workspace">
      <div className="heading"><h1>Convert a table</h1><span className="format">CSV / TSV → JSON</span></div>
      ...
      {source&&<div className="actions"><p>Originals stay untouched.</p><button className="primary" disabled={busy} onClick={()=>run(async()=>{const result=await window.fileform.saveJSON(source.id);if(result)setSaved(result);})}>{busy?'Processing…':'Save JSON…'}</button></div>}
