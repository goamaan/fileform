...
fn main() {
    ...
    let request = match args.as_slice() {
        ...
        {
            let allow_missing_text = match options {
                [] => false,
                [flag] if flag == "--allow-missing-text" => true,
                _ => {
                    eprintln!("Expected --allow-missing-text or no option.");
                    std::process::exit(2)
                }
            };
